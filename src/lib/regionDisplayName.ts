/**
 * Human-readable region labels for report KPIs and tables.
 * Merchant extract mixes ISO 3166-1 alpha-2 and alpha-3; alpha-3 is mapped where needed, then {@link Intl.DisplayNames}.
 */

const ISO3166_ALPHA3_TO_ALPHA2: Record<string, string> = {
  ...
  ROM: "RO", RUS: "RU", RWA: "RW", SAU: "SA", SDN: "SD", SEN: "SN", SGP: "SG", SGS: "GS", SHN: "SH",
  SJM: "SJ", SLB: "SB", SLE: "SL", SLV: "SV", SMR: "SM", SOM: "SO", SPM: "PM", SRB: "RS", SSD: "SS",
  STP: "ST", SUR: "SR", SVK: "SK", SVN: "SI", SWE: "SE", SWZ: "SZ", SXM: "SX", SYC: "SC", SYR: "SY",
  TCA: "TC", TCD: "TD", TGO: "TG", THA: "TH", TJK: "TJ", TKL: "TK", TKM: "TM", TLS: "TL", TON: "TO",
  TTO: "TT", TUN: "TN", TUR: "TR", TUV: "TV", TWN: "TW", TZA: "TZ", UGA: "UG", UKR: "UA", UMI: "UM",
  URY: "UY", USA: "US", UZB: "UZ", VAT: "VA", VCT: "VC", VEN: "VE", VGB: "VG", VIR: "VI", VNM: "VN",
  VUT: "VU", WLF: "WF", WSM: "WS", YEM: "YE", ZAF: "ZA", ZMB: "ZM", ZWE: "ZW",
};

function toAlpha2(code: string): string | null {
  const t = code.trim();
  if (!t || t === "Unknown / Null") return null;
  const u = t.toUpperCase();
  if (u.length === 2) return u;
  if (u.length === 3) return ISO3166_ALPHA3_TO_ALPHA2[u] ?? null;
  return null;
}

/** English display name for a merchant or user country code from the extract. */
export function regionDisplayName(code: string): string {
  const raw = (code || "").trim();
  if (!raw || raw === "Unknown / Null") return raw || "—";

  const a2 = toAlpha2(raw);
  if (!a2) return raw;

  try {
    const label = new Intl.DisplayNames(["en-GB"], { type: "region" }).of(a2);
    return label ?? raw;
  } catch {
    return raw;
  }
